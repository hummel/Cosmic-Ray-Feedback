...
\end{quote}\subsection{Column Density Estimation}
...
In order to estimate $N$, we calculate the column density approaching the center of the accretion disk along both polar and equatorial lines of sight. As shown in Figure 1, the colum density remains essentially constant for the duration of the simulation, and is nearly linear in log-space over several orders of magnitude up to the resolution limit of our simulation ($\sim$100\au$). This same linear behaviour can be seen versus density as well, as shown in Figure 2. In addition we notice that for a given gas density, the column density along the pole is roughly a factor of 10 lower than along the equator.  